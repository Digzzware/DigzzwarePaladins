...
float smoothingValue = 0.25f; // from 0-1
...
void ESPLoop() {
	...
	auto thirdpersontoggle = "";
	auto esptoggle = "";
	auto aimsmoothingtoggle = "";
	auto predictiontoggle = "";


	
	if(thirdPerson == false) {
		thirdpersontoggle = "THIRDPERSON: off";
	}
	else if(thirdPerson == true) {
		thirdpersontoggle = "THIRDPERSON: on";
	}

	if (nameESP == false) {
		esptoggle = "ESP: off";
	}
	else if (nameESP == true) {
		esptoggle = "ESP: on";
	}

	if (smoothing == false) {
		aimsmoothingtoggle = "AimSmoothing: off";
	}
	else if (smoothing == true) {
		aimsmoothingtoggle = "AimSmoothing: on";
	}
	if (smoothing == false) {
		aimsmoothingtoggle = "AimSmoothing: off";
	}
	...
	if (isPredictionAim == false) {
		predictiontoggle = "PredictionAim: off";
	}
	else if (isPredictionAim == true) {
		predictiontoggle = "PredictionAim: on";
	}


	// this is under the GPL-v3, do not sell this and give credit
	g_overlay->draw_text(5, 5, D2D1::ColorF(0.00f, 0.99f, 0.00f), thirdpersontoggle);
	g_overlay->draw_text(5, 15, D2D1::ColorF(0.00f, 0.99f, 0.00f), esptoggle);
	g_overlay->draw_text(5, 25, D2D1::ColorF(0.00f, 0.99f, 0.00f), aimsmoothingtoggle);
	g_overlay->draw_text(5, 35, D2D1::ColorF(0.00f, 0.99f, 0.00f), predictiontoggle);
}
...
static void render(FOverlay* overlay)
{
	while(rendering)
	{
		...
		if (GetAsyncKeyState(VK_F1) && GetTickCount() - lastpress > 500) // THIRD PERSON
		{
			lastpress = GetTickCount();
			thirdPerson = !thirdPerson;
		}

		if (GetAsyncKeyState(VK_F2) && GetTickCount() - lastpress > 500) // ESP
		{
			lastpress = GetTickCount();
		...
		if (GetAsyncKeyState(VK_F3) && GetTickCount() - lastpress > 500) // AIM SMOOTHING
		{
			lastpress = GetTickCount();
			smoothing = !smoothing;
		}

		if (GetAsyncKeyState(VK_F4) && GetTickCount() - lastpress > 500) // AIM PREDICTION
		{
			lastpress = GetTickCount();
		...
		if (GetAsyncKeyState(VK_F8) && GetTickCount() - lastpress > 500) // EXIT
		{
